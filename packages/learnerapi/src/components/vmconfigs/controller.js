...
const setScenarioLearnerConfiguration = ({ dao, db }) =>
  async (req, res, next) => {
    try {
      ...
      if (!scenarioid || !learnerid || !scenariolearnersessionid) {
        return res.status(400).send({
          statusCode: 400,
          message: "scenarioid,scenariolearnersessionid and learnerid is required."
        });
      }
...
const updateCompleteTerminate = ({ dao, db }) => async (req, res, next) => {
  ...
  const stopAndDestroyFailedScenarios = ({ dao, db }) => async (req, res, next) => {
    try {
      const ipAddress = req.headers["x-forwarded-for"] || req.connection.remoteAddress;
...
const generateProxmoxAccessToken = ({ dao, db }) => async (req, res) => {
    try {
...
const getOperationFailedLogs = ({ dao, db }) => async (req, res, next) => {
    try {
      const result = await dao.getOperationFailedLogs({ db })();
      return res.status(200).send({statusCode: 200, message: "Logs Fetched Successfully", data: result});
    } catch (err) {
      console.error("Error in getOperationFailedLogs:", err.message);
      return res.status(500).send({statusCode: 500, message: "Error in Fetching Logs"});
    }
  };
...
const vncProxyConsole = ({ dao, db, validation }) => async (req, res) => {
  try {
