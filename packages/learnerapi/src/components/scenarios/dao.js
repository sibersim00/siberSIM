const getAll = ({ db }) => async (learner_sessionid) => {
    try {
...
const getByID = ({ db }) => async (scenarioUUID, learner_id) => {
  try {
...
const startScenario = ({ db, validation }) => async (body) => {
    try {
      const [inProgress] = await db.sequelize.query(`SELECT scenarioid, status FROM scenario_learner  WHERE learner_id = :learner_id AND status IN ('Initializing', 'Running')`,
        {
      ...
      if (inProgress) {
        return {statusCode: 400, message: validation.messages.ONE_ACTIVE_SCENARIO};
      }
      const [existing] = await db.sequelize.query(`SELECT scenariolearnerid FROM scenario_learner WHERE scenarioid = :scenarioid AND learner_id = :learner_id LIMIT 1`,
        {
          replacements: {scenarioid: body.scenarioid, learner_id: body.learner_id},
          type: db.sequelize.QueryTypes.SELECT,
        }
      );
      ...
      if (existing) {
        await db.sequelize.query(`UPDATE scenario_learner SET status = 'Initializing', modifiedon = CURRENT_TIMESTAMP WHERE scenarioid = ? AND learner_id = ?`,
          {
        ...
        await db.sequelize.query(`INSERT INTO scenario_learner (scenariolearneruuid, scenarioid, learner_id, instructor_id)VALUES (UUID(), ?, ?, ?)`,
          {
            replacements: [body.scenarioid, body.learner_id, body.instructor_id],
            type: db.sequelize.QueryTypes.INSERT,
          }
        );
        const [idResult] = await db.sequelize.query(`SELECT LAST_INSERT_ID() AS scenariolearnerid`,
          {
            type: db.sequelize.QueryTypes.SELECT,
          }
        );
      ...
      await db.sequelize.query(`INSERT INTO scenario_learner_session (scenariolearnersessionuuid, scenariolearnerid, scenarioid, learner_id, timer, status) VALUES (?, ?, ?, ?, ?, ?)`,
        {
          replacements: [scenariolearnersessionuuid, scenariolearnerid, body.scenarioid, body.learner_id, body.timer, body.status],
          type: db.sequelize.QueryTypes.INSERT,
        }
      );
      const [sessionResult] = await db.sequelize.query(`SELECT LAST_INSERT_ID() AS scenariolearnersessionid`,
        {
          type: db.sequelize.QueryTypes.SELECT,
        }
      );
      scenariolearnersessionid = sessionResult?.scenariolearnersessionid;
      await db.sequelize.query(`UPDATE scenario_learner SET currentsession_id = ? WHERE scenariolearnerid = ?`,
        {
      ...
      await insertLog(db, {scenarioid: body.scenarioid, learner_id: body.learner_id, scenariolearnerid, status: "Start", scenariolearnersessionid, instructor_id: body.instructor_id || null, type: "Learner", remark: "Scenario started by learner"});
      return {statusCode: 200, message: validation.messages.CONFIGURATION_STARTED, scenariolearnerid, scenariolearnersessionid, scenariolearnersessionuuid};
    } catch (error) {
...
async function insertLog(db, logData) {
  ...
  const logParams = [logData.scenariolearnersessionid, logData.scenarioid, logData.learner_id, logData.scenariolearnerid, logData.instructor_id, logData.type, logData.remark, logData.status];
...
const updateSessionStatus = ({ db, validation }) => async (body) => {
    try {
      await db.sequelize.query(`UPDATE scenario_learner_session SET status = ?, modifiedon = CURRENT_TIMESTAMP, terminatedon = CASE WHEN ? = 'Terminated' THEN CURRENT_TIMESTAMP ELSE terminatedon END, completedon = CASE WHEN ? = 'Completed' THEN CURRENT_TIMESTAMP ELSE completedon END, startedon = CASE WHEN ? = 'Start' AND startedon IS NULL THEN CURRENT_TIMESTAMP ELSE startedon END, resumeon = CASE WHEN ? = 'Resume' THEN CURRENT_TIMESTAMP ELSE resumeon END, timer = CASE WHEN ? IN ('Pause', 'Completed', 'Terminated') THEN ? ELSE timer END, isnotitermination = CASE WHEN ? IN ('Completed', 'Terminated') THEN 'No' ELSE isnotitermination END WHERE scenariolearnersessionid = ?`,
        {
          replacements: [body.status, body.status, body.status, body.status, body.status, body.status, body.timer, body.status, body.scenariolearnersessionid],
          type: db.sequelize.QueryTypes.UPDATE,
        }
      );
      insertLog(db, {scenarioid: body.scenarioid, learner_id: body.learner_id, scenariolearnerid: body.scenariolearnerid, status: body.status, scenariolearnersessionid: body.scenariolearnersessionid, instructor_id: null,
        type: "Learner",
        remark: `Scenario status changed to ${body.status} by learner`,
      });
      if (body.status == "Terminated" || body.status == "Completed") {
        await db.sequelize.query(`UPDATE scenario_learner SET status = ?, modifiedon = CURRENT_TIMESTAMP WHERE scenariolearnerid = ?`,
          {
      ...
      return {statusCode: 200, message: validation.messages.CONFIGURATION_STARTED, scenariolearnerid: body.scenariolearnerid, scenariolearnersessionid: body.scenariolearnersessionid};
    } catch (error) {
      console.error("Error updating scenario learner session status/ scenario learner status:",error);
      throw error;
    }
  };

const getSessionStatus = ({ db }) => async (scenariolearnersessionuuid) => {
    try {
      const [result] = await db.sequelize.query(`SELECT vm_steps FROM scenario_learner_session WHERE scenariolearnersessionuuid = ?`,
        {
...
const getMessagesByScenario = ({ db, validation }) => async ({ scenariolearnerid }) => {
    ...
    try {
      ...
      await db.sequelize.query(`UPDATE scenario_learner_chats SET status = 'seen' WHERE scenariolearnerid = ? AND sender_type IN ('Admin', 'Instructor') AND status = 'sent'`,
        {
          replacements: [scenariolearnerid],
          type: db.sequelize.QueryTypes.UPDATE,
        }
...
const sendMessage = ({ db, validation }) => async (body, sender_id) => {
  try{
    const {scenariolearnerid, scenarioid, learner_id, instructor_id, sender_type, message, attachment = null} = body;
    await db.sequelize.query(`INSERT INTO scenario_learner_chats (scenariolearnerid,scenarioid, learner_id, instructor_id,sender_type, message, attachment, status, createdon) VALUES (?,?, ?, ?, ?, ?, ?, 'sent', CURRENT_TIMESTAMP)`,
      {
        replacements: [scenariolearnerid, scenarioid, learner_id, instructor_id, sender_type, message, attachment],
        type: db.sequelize.QueryTypes.INSERT,
      }
    );
    const [result] = await db.sequelize.query(`SELECT * FROM scenario_learner_chats WHERE scenariolearnerchatid = LAST_INSERT_ID()`,
      { type: db.sequelize.QueryTypes.SELECT }
    );
    return {statusCode: 200, message: validation.messages.MESSAGE_SENT, data: result};
  } catch (error) {
...
const getLogs = ({ db }) => async (scenariouuid, learner_id) => {
    try {
      let result = await db.sequelize.query(`SELECT sll.status, DATE_FORMAT(sll.createdon, '%Y-%m-%d %H:%i:%s') AS createdon, sll.type, sll.remark, IFNULL(DATE_FORMAT(sls.startedon, '%Y-%m-%d %H:%i:%s'), '') AS startedon FROM scenario_learner_logs sll INNER JOIN scenarios s ON s.scenarioid = sll.scenarioid INNER JOIN scenario_learner_session sls ON sls.scenariolearnersessionid = sll.scenariolearnersessionid WHERE s.scenariouuid = ? AND sll.learner_id = ? ORDER BY sll.createdon DESC;`,
        {
