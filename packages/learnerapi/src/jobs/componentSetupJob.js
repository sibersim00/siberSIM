...
async function componentSetupJob(db, ipAddress, { scenarioid, learnerid, scenariolearnersessionid }) {
  console.log(`Starting component setup job for Scenario: ${scenarioid}, Learner: ${learnerid}`);

  const statusVal = "Initializing";

  try {
    const componentConfig = await db.sequelize.query(`SELECT vmconfigurationid, componentid, \`order\`, vmid AS clone_vmid,  componentname AS name, duration, componenttype, master_vmid AS source_vmid FROM vm_configuration WHERE scenariolearnersessionid = ? AND status = ?`,
      {
    ...
    if (componentConfig.length == 0) {
      ...
      await handleComponentFailure( db, scenarioid, learnerid, scenariolearnersessionid, statusVal, ERROR_MESSAGES.CONFIG_NOT_FOUND);
    ...
    await db.sequelize.query(`INSERT INTO scenario_learner_logs (scenariolearnersessionid, scenarioid, learner_id, scenariolearnerid, type, remark, status, createdon) SELECT sls.scenariolearnersessionid, sls.scenarioid, sls.learner_id, sls.scenariolearnerid, 'System', ?, 'Initiated', NOW() FROM scenario_learner_session sls WHERE sls.scenariolearnersessionid = ?`,
      {
    ...
    for (const component of componentConfig) {
      try {
        ...
        await cloneComponentVM(db, ipAddress, component, scenariolearnersessionid);
      } catch (err) {
        const reason = `${ERROR_MESSAGES.UNHANDLED_SETUP_ERROR}: ${err.message}`;
        console.error(reason);
        await handleComponentFailure(db, scenarioid, learnerid, scenariolearnersessionid, statusVal, reason);
      }
    }
    ...
    await db.sequelize.query(`UPDATE scenario_learner_session SET vm_steps = 'Cloning', modifiedon = NOW() WHERE scenariolearnersessionid = ?`,
      {
    ...
    const updatedComponents = await db.sequelize.query(`SELECT vmconfigurationid, vmid, componenttype, componentname AS name, network_bridge_json, status FROM vm_configuration WHERE scenariolearnersessionid = ? AND status = 'Cloning'`,
      {
        replacements: [scenariolearnersessionid],
        type: db.sequelize.QueryTypes.SELECT,
      }
    ...
    await configureComponentVM(db, ipAddress, {scenarioid, learnerid, scenariolearnersessionid, components: updatedComponents});
    ...
    const componentsForStart = await db.sequelize.query(`SELECT vmconfigurationid, vmid, componenttype, componentname AS name, network_bridge_json, status FROM vm_configuration WHERE scenariolearnersessionid = ? AND status = 'Bridge Configuration'`,
      {
        replacements: [scenariolearnersessionid],
        type: db.sequelize.QueryTypes.SELECT,
      }
    ...
    await startComponentVM(db, ipAddress, { scenarioid, learnerid, scenariolearnersessionid, components: componentsForStart,});
  } catch (err) {
    const reason = `Unhandled error in component setup: ${err.message}`;
    console.error(reason);
    await stopAndDestroyComponentVM(db, ipAddress, { scenarioid, learnerid, scenariolearnersessionid,});
    await handleComponentFailure(db, scenarioid, learnerid, scenariolearnersessionid, statusVal, reason);
  }
}

async function cloneComponentVM(db, ipAddress, component, scenariolearnersessionid) {
  ...
  if (!clone_vmid || !source_vmid) {
    ...
    console.error(reason);
    await handleComponentFailure(db, scenarioid, learner_id, scenariolearnersessionid, statusVal, reason);
  ...
  try {
    ...
    await handleComponentFailure(db, scenarioid, learner_id, scenariolearnersessionid, statusVal, `${ERROR_MESSAGES.CLONE_FAILED} (${name}): ${err.message}`);
    throw err;
  }
}

async function configureComponentVM(db, ipAddress, { scenarioid, learnerid, scenariolearnersessionid, components }) {
  const statusVal = "Cloning";
  try {
    const filteredComponents = components.filter((c) => c.status === "Cloning");
    if (!filteredComponents || filteredComponents.length === 0) {
      await handleComponentFailure(db, scenarioid, learnerid, scenariolearnersessionid, statusVal, ERROR_MESSAGES.NO_COMPONENTS_FOR_STATUS);
      return {
        success: false,
    ...
    for (const component of filteredComponents) {
      ...
      try {
        ...
        await handleComponentFailure(db, scenarioid, learnerid, scenariolearnersessionid, statusVal, `${ERROR_MESSAGES.CONFIGURATION_ERROR} (${component.name}): ${err.message}`);
    ...
    await db.sequelize.query(`UPDATE scenario_learner_session SET vm_steps = 'Bridge Configuration', modifiedon = NOW() WHERE scenariolearnersessionid = ?`,
      {
        replacements: [scenariolearnersessionid],
        type: db.sequelize.QueryTypes.UPDATE,
      }
    ...
    await handleComponentFailure(db, scenarioid, learnerid, scenariolearnersessionid, statusVal, `${ERROR_MESSAGES.UNHANDLED_CONFIGURE_ERROR}: ${err.message}`);
  }
}

async function startComponentVM(db, ipAddress, { scenarioid, learnerid, scenariolearnersessionid, components }) {
  ...
  try {
    ...
    for (const component of filteredComponents) {
      ...
      try {
        ...
        await handleComponentFailure(db, scenarioid, learnerid, scenariolearnersessionid, statusVal, `${ERROR_MESSAGES.START_ERROR} (${component.name}): ${err.message}`);
        return {
          success: false,
    ...
    await db.sequelize.query(`UPDATE scenario_learner_session SET vm_steps = 'Running', modifiedon = NOW(),startedon = CURRENT_TIMESTAMP WHERE scenariolearnersessionid = ?`,
      {
        replacements: [scenariolearnersessionid],
        type: db.sequelize.QueryTypes.UPDATE,
      }
    ...
    await db.sequelize.query(`UPDATE scenario_learner_session SET status = 'Start', modifiedon = NOW() WHERE scenariolearnersessionid = ? AND vm_steps = 'Running'`,
      {
        replacements: [scenariolearnersessionid],
        type: db.sequelize.QueryTypes.UPDATE,
      }
    ...
    if (scenarioData && scenarioData.scenariodiagram) {
      ...
      try {
        ...
        await handleComponentFailure(db, scenarioid, learnerid, scenariolearnersessionid, statusVal, `${ERROR_MESSAGES.UNHANDLED_START_ERROR}: Failed to parse scenario diagram`);
        return;
      }
      ...
      const componentDetails = await db.sequelize.query(`SELECT vmid, componentname,nodeid,componenttype  FROM vm_configuration WHERE scenariolearnersessionid = ? AND scenarioid = ? AND learner_id = ?`,
        {
          replacements: [scenariolearnersessionid, scenarioid, learnerid],
          type: db.sequelize.QueryTypes.SELECT,
        }
      ...
      await db.sequelize.query(`UPDATE scenario_learner_session SET scenariodiagram = ?, modifiedon = NOW() WHERE scenariolearnersessionid = ?`,
        {
      ...
      await handleComponentFailure(db, scenarioid, learnerid, scenariolearnersessionid, statusVal, `${ERROR_MESSAGES.UNHANDLED_START_ERROR}: Scenario diagram not found`);
    }
  } catch (err) {
    console.error("Unhandled error in startComponentVM:", err);
    await handleComponentFailure(db, scenarioid, learnerid, scenariolearnersessionid, statusVal, `${ERROR_MESSAGES.UNHANDLED_START_ERROR}: ${err.message}`);
  }
}

async function stopAndDestroyComponentVM(db, ipAddress, { scenarioid, learnerid, scenariolearnersessionid }) {
  ...
  try {
    ...
    for (const component of components) {
      ...
      try {
        ...
        await handleComponentFailure(db, scenarioid, learnerid, scenariolearnersessionid, statusVal, ERROR_MESSAGES.COMPONENT_OPERATION_FAILED);
        throw err;
      }
    }
    await handleComponentFailure(db, scenarioid, learnerid, scenariolearnersessionid, statusVal, ERROR_MESSAGES.CLEANUP_COMPLETED_WITH_FAILURE);
...
async function handleComponentFailure( db, scenarioid, learnerid, scenariolearnersessionid, currentStatus, reason) {
  ...
  await db.sequelize.query(`UPDATE scenario_learner_session SET vm_steps = 'Failed', modifiedon = NOW(),failedon  = NOW() WHERE scenariolearnersessionid = ?`,
    {
      replacements: [scenariolearnersessionid],
      type: db.sequelize.QueryTypes.UPDATE,
    }
  );
  await db.sequelize.query(`UPDATE scenario_learner_session SET status = 'Failed', modifiedon = NOW() WHERE scenariolearnersessionid = ?`,
    {
      replacements: [scenariolearnersessionid],
      type: db.sequelize.QueryTypes.UPDATE,
    }
  );
  await db.sequelize.query(`UPDATE scenario_learner SET status = 'Terminated', modifiedon = NOW() WHERE scenariolearnerid = ( SELECT scenariolearnerid FROM scenario_learner_session WHERE scenariolearnersessionid = ?)`,
    {
      replacements: [scenariolearnersessionid],
      type: db.sequelize.QueryTypes.UPDATE,
    }
  ...
  await db.sequelize.query(`INSERT INTO scenario_learner_logs (scenariolearnersessionid, scenarioid, learner_id, scenariolearnerid, type, remark, status, createdon) SELECT sls.scenariolearnersessionid, sls.scenarioid, sls.learner_id, sls.scenariolearnerid, 'System', ?, 'Failed', NOW() FROM scenario_learner_session sls WHERE sls.scenariolearnersessionid = ?`,
    {
