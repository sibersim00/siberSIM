...
async function createVNCProxy(vmid, vmType) {
  if (!accessInfo?.cookie || !accessInfo?.CSRFPreventionToken) {
    throw new Error(
      "Access info not initialized. Call generateAccessTicket first."
    );
  ...
  const start = Date.now();
  const request_datetime = new Date();
  const url = `${constants.endpoint}/nodes/${constants.current_node}/${vmType}/${vmid}/vncproxy`;

  const config = {
    method: "post",
    url,
    headers: {
      Cookie: accessInfo.cookie, // e.g., PVEAuthCookie=...
      "Content-Type": "application/x-www-form-urlencoded",
      CSRFPreventionToken: accessInfo.CSRFPreventionToken,
    },
    httpsAgent: new https.Agent({ rejectUnauthorized: false }),
    data: "", // Required for POST without params
  };
  ...
  try {
    ...
    return response.data; // Usually returns a ticket + port
  } catch (error) {
    const errorCode = error?.response?.status?.toString() || "ERR";
    const errorMessage = error?.response?.data || error.toString();
    await logApiRequestData(
    ...
    console.error("Error creating VNC Proxy:", errorMessage);
    return {
      status: errorCode,
      data: null,
      error: errorMessage,
...
async function openVNCConsole(vncUrl) {
  if (!accessInfo?.cookie || !accessInfo?.CSRFPreventionToken) {
    throw new Error(
      "Access info not initialized. Call generateAccessTicket first."
    );
  ...
  const config = {
    method: "get", // GET request for the console URL
    url: vncUrl,
    headers: {
      Cookie: accessInfo.cookie, // PVEAuthCookie
      "Content-Type": "application/x-www-form-urlencoded",
      CSRFPreventionToken: accessInfo.CSRFPreventionToken,
  ...
  try {
    ...
    return response.data; // usually the HTML or redirect info for NoVNC
  } catch (error) {
    const errorCode = error?.response?.status?.toString() || "ERR";
    const errorMessage = error?.response?.data || error.toString();
    await logApiRequestData(
    ...
    console.error("Error opening VNC Console:", errorMessage);
    return {
      status: errorCode,
      data: null,
      error: errorMessage,
