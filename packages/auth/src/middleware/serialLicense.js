...
function checkValidate(hostname, licenseKey) {
  try {
    ...
    const startStr = parts[0].replace("S", "");   // S20260215
    const userPart = parts[1];                    // UL2M1   <-- UPDATED
    const expiryStr = parts[2].replace("E", "");  // E20260216
    const hostnameStr = parts[3];                 // 19C9ED
    const sentHash = parts[4];                    // 4DDDD3
    const match = userPart.match(/UL(\d+)M([01])/);
    if (!match) {
      return { isKeyValid: false };
    }
    const user_count = match[1];        // e.g. "2"
    const manipulation = match[2];      // "0" or "1"
    ...
    const todayUTC = new Date();
    todayUTC.setUTCHours(0, 0, 0, 0);
    const strDate = new Date(Date.UTC(startStr.substring(0, 4),startStr.substring(4, 6) - 1,startStr.substring(6, 8)));
    ...
    // Expiry date check (UNCHANGED)
    const expDate = new Date(Date.UTC(expiryStr.substring(0, 4),expiryStr.substring(4, 6) - 1,expiryStr.substring(6, 8)));
