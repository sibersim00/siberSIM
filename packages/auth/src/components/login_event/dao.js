...
const checklogin = ({ db, keys, validation }) => async ({ loginid, password, eventid }) => {
  try {
...
const verifylogin = ({ db }) => async ({ loginid, password, otp, eventid }) => {
  loginid = loginid.trim();
  password = password.trim()
  const [learner] = await db.sequelize.query(`SELECT
  l.learner_id,
  l.learner_uuid,
  l.firstname,
  l.lastname,
...
WHERE
  ...
  if (isMatch == true) {
    delete learner.password;
    if (learner) {
      await db.sequelize.query(`UPDATE learners set otp=null, otptimeout=null where learner_id = ? and status ='Active'`, { replacements: [learner.learner_id] });
      // return learner;
...
const verifyDirectLogin = ({ db, validation }) => async ({ loginid, password, eventid }) => {
  try {
    loginid = loginid.trim();
    password = password.trim()
    ...
    if (!learner) {
      return { statusCode: 404, message: validation?.messages?.invalid_credentials || "Invalid credentials" };
    }
    const now = new Date();
    const start = new Date(learner.eventstarttime);
    const end = new Date(learner.eventendtime);
