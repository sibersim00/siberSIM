...
// const generateLicense = ({ start_date, user_count, expiry_date, domain_name,manipulation }) => {
...
const generateLicense = ({
  ...
  const manipulationBit =
    manipulation === true || manipulation === "True" || manipulation === 1
      ? 1
      : 0;
  const parsedExp = new Date(expiry_date);
  if (isNaN(parsedExp.getTime())) throw new Error("Invalid expiry date");
  ...
  const formattedStr = parsedStr.toISOString().slice(0, 10).replace(/-/g, "");
  const domainRaw = `${domain_name}|${formattedExp}`;
  const encryptedDomain = CryptoJS.SHA256(domainRaw)
    .toString()
    .substring(0, 6)
    .toUpperCase();
  const raw = `${formattedStr}|${user_count}|${manipulationBit}|${formattedExp}|${encryptedDomain}|${SECRET}`;
  const shortHash = CryptoJS.SHA256(raw)
    .toString()
    .substring(0, 6)
    .toUpperCase();
  return `S${formattedStr}-UL${user_count}M${manipulationBit}-E${formattedExp}-${encryptedDomain}-${shortHash}`;
};
...
function validateLicense(hostname, licenseKey) {
  try {
    ...
    const encryptedDomain = CryptoJS.SHA256(domainRaw).toString().substring(0, 6).toUpperCase();
    if (encryptedDomain !== hostnameStr) {
      throw new Error("Invalid License");
    }
    ...
    let expDate = new Date(
      Date.UTC(expiryStr.substring(0, 4),expiryStr.substring(4, 6) - 1,expiryStr.substring(6, 8),),
    );
    ...
    let strDate = new Date(
      Date.UTC(startStr.substring(0, 4),startStr.substring(4, 6) - 1,startStr.substring(6, 8),),
    );
...
//     // Build RAW — must match generator EXACTLY
//     const raw = `${startStr}|${user_count}|${expiryStr}|${hostnameStr}|${SECRET}`;
//     const calculatedHash = CryptoJS.SHA256(raw)
//       .toString()
//       .substring(0, 6)
//       .toUpperCase();
//     if (calculatedHash !== sentHash) {
...
function checkValidate(hostname, licenseKey) {
  try {
    ...
    const match = userPart.match(/UL(\d+)M([01])/);
    if (!match) {
      return { isKeyValid: false };
    }
    const user_count = match[1]; // "2"
    const manipulation = match[2];
    // Build RAW — must match generator EXACTLY
    // const raw = `${startStr}|${user_count}|${expiryStr}|${hostnameStr}|${SECRET}`;
    const raw = `${startStr}|${user_count}|${manipulation}|${expiryStr}|${hostnameStr}|${SECRET}`;
    const calculatedHash = CryptoJS.SHA256(raw)
