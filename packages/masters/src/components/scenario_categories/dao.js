const getscenarioAll = ({ db }) => async (id = null) => {
  try {
    let [res] = await db.sequelize.query(`SELECT sc.categoryname, sc.categoryimage,sc.parentscenariocategoryid, sc.scenariocategoryid,
...
const statusChange = ({ db,validation }) => async (body) => {
 ...
 await db.sequelize.query(`UPDATE scenario_categories set status = '${status}', modifiedby = '${body.loginId}', modifiedon = now() where scenariocategoryid = :_id`, {
    replacements: { _id: body.scenariocategoryid }
  });
  return { 'statusCode': 200, 'message': validation.messages.status_change };
};

const getScenarioCategorybyId = ({ db }) => async (id) => {
  try {
  ...
  try {
    await db.sequelize.query(
      `INSERT INTO scenario_categories (
        scenariocategoryuuid, categoryname, categoryimage,
        createdby, createdon
      ) VALUES (
        uuid(), ?, ?, ?, CURRENT_TIMESTAMP
      )`,
      {
        replacements: [body.categoryname, body.categoryimage, userid],
...
const deleteById = ({ db, validation }) => async (id = null) => {
  let [a] = await db.sequelize.query(`SELECT scenariocategoryid, parentscenariocategoryid FROM scenario_categories WHERE deletedon IS NULL and parentscenariocategoryid='${id}'`);
  if (a.length > 0) {
    return { status: false, message: "Scenario Category already mapped with scenario Sub-Category" };
  }
   await db.sequelize.query('UPDATE scenario_categories set deletedon=now() where scenariocategoryid=:_id', {
    replacements: { _id: id }
  });
  await db.sequelize.query('UPDATE scenario_categories set deletedon=now() where parentscenariocategoryid=:_id', {
    replacements: { _id: id }
  });
  return { status: true, message: "Record has been Deleted Successfully." };
};

const scenariocategoryverify = ({ db, validation }) => async (body) => {
  const errors = [];
  const success = [];
    ...
    for (const element of body) {
      ...
      if (!asset.categoryname) {
        ...
        if (!/^[a-zA-Z\s-]+$/.test(asset.categoryname)) {
          scenariocategoryerror.push({
            field: "categoryname",
            message: "Invalid: Only alphabets, hyphens, and spaces allowed in category name."
          });
        }
      ...
      if (asset.subcategoryname) {
        if (emojiRegex.test(asset.subcategoryname)) {
          ...
          const checkSubCategoryResult = await db.sequelize.query(checkSubCategoryQuery, {
      ...
      if (asset.scenariocategoryid) {
        ...
        if (emojiRegex.test(idStr)) {
          ...
          const checkScenarioValues = { scenariocategoryid: asset.scenariocategoryid };
          const checkScenarioResult = await db.sequelize.query(checkScenarioQuery, {
...
const scenariocategoryImport = ({ db, validation }) => async (body, user) => {
  try {
    ...
    for (const asset of body) {
      ...
      if (asset.scenariocategoryid && asset.scenariocategoryid !== "" && asset.scenariocategoryid !== 0) {
        const updateQuery = `
          UPDATE scenario_categories 
          SET categoryname = ?, parentscenariocategoryid =?, modifiedby = ?, modifiedon = CURRENT_TIMESTAMP 
          WHERE scenariocategoryid = ?`;

        const [updatedScenariocategory] = await db.sequelize.query(updateQuery, {
          replacements: [
            asset.categoryname || '', 
        ...
        if (updatedScenariocategory) {
          ...
          const [newScenariosubcategory] = await db.sequelize.query(insertQuery, {
