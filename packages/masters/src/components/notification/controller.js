...
const getNotificationAll = ({ dao, db}) => async (req, res) => {
  try {
    ...
    const userid = req.params.flag == type ? req.user.userid : req.user.userid;
...
const UpdateReadNotification = ({ dao, db,validation}) => async (req, res) => {
  try {
    let body=req.body;
    const { error, value } = validation.notischema.validate(body,{ abortEarly: false,allowUnknown:true });
    if (error) {
      const errors = error.details.map((err) => err.message);
      return res.status(400).json({statusCode:400, errors:errors});
    } else {
      body.userid = body.type == "User" ? req.user.userid : req.user.tutor_id; 
