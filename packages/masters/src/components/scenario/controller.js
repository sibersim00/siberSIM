...
const list =
  ...
  async (req, res) => {
    try {
      ...
      const manipulation = req.user.manipulation;
      const result = await dao.list({ db })(usertype, session_userid,manipulation);
      res
        .status(200)
        .send({ statusCode: 200, message: "Get Scenario List", manipulation,data: result });
    } catch (error) {
...
const changeMaipulationStatus =
  ({ dao, db, validation }) =>
  async (req, res) => {
    try {
      let body = req.body;
      const session_userid = req.user.userid;
      const result = await dao.changeMaipulationStatus({ db, validation })(
        body,
        session_userid,
      );
      res
  ...
  async (req, res) => {
    try {
      ...
      const exportDir = path.join(
        tempZipDir,
        `scenario_${scenarioid}_${Date.now()}`,
      );
