  ...
  ({ db }) =>
    async (id = null, ipAddress = null) => {
      try {
        ...
        SELECT   c.componentid,  c.componentuuid,  c.vmid,  c.vmid_name,  c.componentname,  c.componentimage,  c.componentcategoryid,  c.componenttype,  c.duration,  c.memory,  c.cores,  c.component_status,  c.storage,  c.network_ports,  mcc.categoryname,  CASE   WHEN c.status = 'Active' THEN 'true'    ELSE 'false'    END AS status,  DATE_FORMAT(c.createdon, '%Y-%m-%d %H:%i:%s') AS createdon,  DATE_FORMAT(c.modifiedon, '%Y-%m-%d %H:%i:%s') AS modifiedon  FROM   components c  JOIN component_categories mcc ON mcc.componentcategoryid = c.componentcategoryid  WHERE   c.deletedon IS NULL  GROUP BY c.componentid  ORDER BY c.componentname ASC
      `);
        return components;
      } catch (error) {
      ...
      `;
        ...
        const extractStorage = () => {
          ...
          if (foundKey) {
            ...
            console.log("sizeMatchsizeMatchsizeMatchsizeMatch", sizeMatch)
            if (sizeMatch) {
  ...
  ({ db, ipAddress }) =>
    async (body, session_userid) => {
      try {
        ...
        //  Check if used in scenarios
        const [used] = await db.sequelize.query(
          `SELECT scenarioid
   FROM scenarios
   WHERE deletedon IS NULL
   AND JSON_SEARCH(component_config, 'one', :vmid, NULL, '$[*].vmid') IS NOT NULL`,
          {
            replacements: { vmid: String(vmId) }, 
          }
        );
        if (used.length > 0) {
          return {
            status: false,
            message:
              "Component is already used in active scenarios and cannot be deleted.",
          };
        }
        const [comp] = await db.sequelize.query(
         ...
         LIMIT 1`,
          { replacements: { vmid: vmId } }
        );

        if (!comp.length) {
          return { status: false, message: "Component not found." };
        }

        const componenttype = comp[0].componenttype;
        const vmType =
          componenttype === "QEMU"
            ? "qemu"
            : componenttype === "LXC"
        ...
        if (!vmType) {
          return { status: false, message: "Invalid component type." };
        }
        ...
        const destroyResp = await proxmoxService.destroyVM(vmId, vmType);
        console.log("Destroy response:", destroyResp?.status);
        if (
          destroyResp?.status &&
          destroyResp.status !== 200 &&
          destroyResp.status !== 204
        ) {
          return {
            status: false,
            message: "Failed to destroy VM in Proxmox.",
          };
        }
        await db.sequelize.query(
          `UPDATE components 
         SET deletedon = NOW(), modifiedby = :userid 
         WHERE vmid = :vmid`,
          {
            replacements: {
              vmid: vmId,
        ...
        return {
          status: true,
          message: "Component destroyed and deleted successfully.",
        };
      } catch (err) {
        console.error("Delete Error:", err);
        throw new Error("Delete failed due to DB or Proxmox error.");
      }
    };
          ...
          `;
            ...
            if (!isDuplicate || isCurrent) {
              vmidMap.set(item.vmid, {
                ...
                subcategoryname: `${item.vmid} - ${isCurrent ? currentVMName : item.name
                  }`,
              });
            }
          }
