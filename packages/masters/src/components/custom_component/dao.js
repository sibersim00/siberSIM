...
const save =
  ...
  `,
          ...
          const extractStorage = () => {
            const qemuStorageKeys = ["sata0", "scsi0", "ide0", "virtio0", "nvme0", "usb0"];
            ...
            if (foundKey) {
              const key = Object.keys(proxmoxData).find((k) => k.toLowerCase() === foundKey);
          ...
          const extractPortsPrefix = () => {
            ...
            Object.entries(proxmoxData).forEach(([key, value]) => {
              ...
              if (body.subcategoryTypeid.toLowerCase() === "qemu") {
                ...
                if (nameMatch && nameMatch[1]) prefix[key] = `name=${nameMatch[1]}`;
              }
            });
            return prefix;
          };
