...
export function getChatMessages(payload) {
  return async (dispatch) => {
    ...
    try {
      const response = await axios.post(`${api.event_chatmessage_get}`, payload);
...
export function getRefreshMessage(payload) {
  return async (dispatch) => {
    dispatch(slice.actions.startLoading());
    try {
      const response = await axios.post(`${api.event_chat_refresh}`, payload);
...
export function chatMessageSeen(payload) {
  return async (dispatch) => {
    dispatch(slice.actions.startLoading());
    try {
      const response = await axios.post(`${api.event_chat_markseen}`, payload);
