...
import { ReactFlow, ReactFlowProvider,useUpdateNodeInternals , addEdge, useNodesState, useEdgesState, Background, Handle, Position, useReactFlow,
} from "@xyflow/react";
...
>
  ...
  const addDraggedCompSucc = useSelector(
    (state) => state?.scenarios?.saveDraggedComponentData?.data,
  );
  ...
  const addNetwordId = useSelector(
    (state) => state?.scenarios?.modifyNetworkIdData?.data,
  );
  const plugNetwork = useSelector(
    (state) => state?.scenarios?.plugNetworkPort?.data,
  );
  const unplugNetwork = useSelector(
    (state) => state?.scenarios?.unplugNetworkPort?.data,
  );
  const connectNetwork = useSelector(
    (state) => state?.scenarios?.connectNetworkPort?.data,
  );
...
const iconMap = {
  ...
  return (
    <div
      ...
      style={{
        width: 320,
        background: "linear-gradient(180deg,#0b1220,#0f1b2e)",
        borderRadius: 16,
        padding: 16,
        color: "#fff",
              ...
              <div className="d-flex justify-content-between align-items-center">
                ...
                >
                  <Button
                    ...
                    onClick={() => {
                      router.push(
                        `/scenarios_view/${scenario?.scenariouuid}
                        `,
                      );
                      dispatch(clearSingleScenarios());
                    }}
