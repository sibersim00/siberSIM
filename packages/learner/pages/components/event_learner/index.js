...
const Home = () => {
  ...
  const { username, eventid, password, otp } = data;
  ...
  useEffect(() => {
    if (eventListData && eventListData.length > 0) {
      let temp = eventListData.map((cat) => ({
        eventname: (cat?.eventname || ''),
        eventid: cat?.eventid
      }));
      setEventListDropDown(temp);
    }
  }, [eventListData]);
  useEffect(() => {
  ...
  // -------------success code for veriefy password---------------------
  useEffect(() => {
  ...
  const LoginTopanel = (e) => {
    ...
    if (data.username == "") {
      toast.error(
        <p className="mx-2 tx-16 d-flex align-items-center mb-0">
          {t("eventLogin.errormsg.please_enter_username")}
        </p>,
        {
          position: toast.POSITION.TOP_RIGHT,
          hideProgressBar: true,
      ...
      toast.error(
        <p className="mx-2 tx-16 d-flex align-items-center mb-0">
          {t("eventLogin.errormsg.please_enter_password")}
        </p>,
        {
          position: toast.POSITION.TOP_RIGHT,
          hideProgressBar: true,
      ...
      if (isLoginSuccess) {
        if (!data.otp) {
          toast.error(
            <p className="mx-2 tx-16 d-flex align-items-center mb-0">
              {t("eventLogin.errormsg.please_enter_otp")}
            </p>,
            {
              position: toast.POSITION.TOP_RIGHT,
              hideProgressBar: true,
          ...
          if (!reg.test(data.otp)) {
            toast.error(
              <p className="mx-2 tx-16 d-flex align-items-center mb-0">
                {t("eventLogin.errormsg.please_enter_valid_otp")}
              </p>,
              {
                position: toast.POSITION.TOP_RIGHT,
                hideProgressBar: true,
            ...
            toast.error(
              <p className="mx-2 tx-16 d-flex align-items-center mb-0">
                {t("eventLogin.errormsg.otp_should_be_digit")}
              </p>,
              {
                position: toast.POSITION.TOP_RIGHT,
                hideProgressBar: true,
  ...
  return (
    <div>
      <Head>
        <title>{t("eventLogin.meta_title")}</title>
                      ...
                      <Card.Body className="mt-2 mb-2">
                        ...
                        <Form className="mb-2">
                          {!isLoginSuccess ? (
                            ...
                            <>
                              <h5 className="text-start mb-2">
                                {t("eventLogin.labels.title1")}
                              </h5>
                              <p className="mb-2 text-muted tx-13 ms-0 text-start">
                                {t("eventLogin.labels.otp_title")}
                                {t("eventLogin.labels.otp_title1")}
                              </p>
                            </>
                          )}

                          <div className="inputgroup-toggles input-group text-start mb-4" style={{ width: "100%" }}>
                            <div className="w-100">
                              <Select
                                name="eventid"
                                placeholder={t("Select Event")}
                                value={eventlistDropDown.find((opt) => opt.eventid === data.eventid) || null}
                                ...
                                options={eventlistDropDown}
                                getOptionLabel={(opt) => opt.eventname}
                                getOptionValue={(opt) => opt.eventid}
                                ...
                                styles={{
                                  control: (base, state) => ({
                                    ...
                                    borderRadius: "50px",
                                    borderWidth: "1px",
                                    borderColor: state.isFocused ? "var(--primary-bg-color)" : "#ced4da",
                                    boxShadow: "none",
                                    backgroundColor: "#fff",
                                    width: "100%", // Make control fill parent
                                    "&:hover": {
                                  ...
                                  option: (base, { isFocused }) => ({
                                    ...base,
                                    backgroundColor: isFocused ? "var(--primary-bg-color)" : "#fff",
                                    color: "#000",
                                  }),
                                  singleValue: (base) => ({
                                    ...base,
                          ...
                          <div className="inputgroup-toggles input-group text-start mb-4">
                            <Form.Control
                              ...
                              placeholder={t("eventLogin.forms.placeholder.username")}
                              name="username"
                              type="text"
                              value={username}
                            ...
                            <>
                              <div className="inputgroup-toggles input-group text-start mb-4">
                                <Form.Control
                                  ...
                                  name="password"
                                  className="form-control ht-45 rounded-50"
                                  placeholder={t("eventLogin.forms.placeholder.password")}
                                  autoComplete="off"
                              ...
                              {!isLoading ? (
                                ...
                                >
                                  {/* {getCompanyListData?.otp_verification == "true" ? (<>{t("eventLogin.forms.button.generate_otp")}</>) : (<>Login</>)} */}
                                  {getCompanyListData?.otp_verification == "true" ? (
                                    <>{t("eventLogin.forms.button.generate_otp")}</>
                                  ) : (
                                    <>Login</>
                                  )}
                                </Button>
                                ...
                                >
                                  ...
                                  />{" "}
                                  <span className="">
                                    {/* {getCompanyListData?.otp_verification == "true" ? (<>{t("eventLogin.forms.button.sending_otp")}</>) : (<>{t("eventLogin.forms.button.verifying")}</>)} */}
                                    {getCompanyListData?.otp_verification == "true" ? (
                                      <>{t("eventLogin.forms.button.sending_otp")}</>
                                    ) : (
                                      <>{t("eventLogin.forms.button.verifying")}</>
                                    )}
                                  </span>
                                </Button>
                                  <br /></>
                            ...
                            <>
                              <div className="inputgroup-toggles input-group text-start">
                                <Form.Control
                                  style={{ paddingLeft: "1.5em" }}
                                  className="form-control ht-45 rounded-50"
                                  placeholder={t("eventLogin.forms.placeholder.otp")}
                                  name="otp"
                                  type="text"
                                  value={otp}
                                  onChange={changeHandler}
                              ...
                              <p className="mb-0 text-right">
                                {timerval <= 0 ? (
                                  ...
                                  >
                                    {t("eventLogin.labels.didn't_recieve_otp")}{" "}
                                    <strong>{t("eventLogin.labels.resend_again")}</strong>
                                  </span>
                                ) : (
                                  <span className="text pointer tx-12 pd-r-10 ">
                                    {t("eventLogin.labels.resent_otp_sec", {
                                      name:
                              ...
                              {!isLoading ? (
                                <Button
                                  ...
                                  className="btn-block ht-45 rounded-50 background-black  text-white"
                                >
                                  {t("eventLogin.forms.button.verify_otp")}
                                </Button>
                              ) : (
                                <Button
                                  // variant="primary"
                                ...
                                >
                                  <Spinner
                                    ...
                                    aria-hidden="true"
                                  />{" "}
                                  <span className=""> {t("eventLogin.forms.button.verifying")}</span>
                                </Button>
                              )}
                              <span
                              ...
                              >
                                {t("eventLogin.labels.sign_diffrent_title")}
                              </span>
                            </>
                          )}
                        </Form>
