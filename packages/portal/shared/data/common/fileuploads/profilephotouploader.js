...
export default function ProfilePhotoUploader({
  ...
  handleUpload: callbackHandleUpload,
  setProfileImage,
  fetchfiles = [],
  folderpath = "profile",
  setFiles,
  ...
  const handleFileChange = async (e) => {
    ...
    if (!file) return;
    // error("");
    // Reset input so selecting same file twice triggers change event
    // e.target.value = "";
    ...
    // if (file.size <= 9 * 1024) {
    //   error("Image size must be at least 8KB.");
    //   toast.error("Image size must be at least 8KB.", {
    //     position: toast.POSITION.TOP_RIGHT,
    //     hideProgressBar: true,
    //     theme: "colored",
    //   });
