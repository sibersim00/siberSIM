...
const EditableEdge = ({ id, sourceX, sourceY, sourcePosition, targetX, targetY, targetPosition, data, markerEnd, allEdges, edgeRouting,
}) => {
  const { setEdges } = useReactFlow();
  const [isEditing, setIsEditing] = useState(false);
  const pathFactory = (edgeRouting || data?.edgeRouting) === 'smooth' ? getSmoothStepPath : getBezierPath;
  ...
  const currentEdge = allEdges.find(e => e.id === id);
  const attackedValue = currentEdge?.isAttacked ?? currentEdge?.data?.isAttacked ?? data?.isAttacked;
  const shouldAnimate =
    attackedValue === true ||
    attackedValue === 1 ||
      ...
      {shouldAnimate && (
        <g className="scenario-network-packet">
          ...
          >
            ...
            <animateMotion dur="2.2s" repeatCount="indefinite" rotate="auto" keyPoints="0;1" keyTimes="0;1" calcMode="linear">
