...
export function changeStatusSubCat(payload,id) {
   return async () => {
      dispatch(slice.actions.startLoading());
      try {
        const response = await axios.post(`${api?.component_subcategories_change_status}`, payload);
...
export function deleteSubCat(payload) {
    return async () => {
      dispatch(slice.actions.startLoading());
      try {
        const response = await axios.post(`${api.component_subcategories_delete}`,payload);
