...
const Normaluser = () => {
  ...
  const isLoading = useSelector((state) => state.normalUSerData?.isLoading);
  useEffect(() => {
    if (typeof window !== "undefined") {
      dispatch(getLocalStorageData("user"));
    }
  ...
  const handleLoadMore = async () => {
    ...
    setPage(nextPage);
    try {
      await dispatch(getNormalusersManageList({ page: nextPage, limit }));
    } finally {
...
}, [observerRef.current, hasMore, isLoadingMore]);
  return (
    <>
      <Seo title="SIMUser" />
      <ToastContainer />
