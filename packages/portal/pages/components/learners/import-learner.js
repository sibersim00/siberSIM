...
function ImportLearners({ impLer, setImpLer }) {
  ...
  const { saveImportLearnerResp, errorData} =
   ...
   useEffect(() => {
    ...
    useEffect(() => {
      if (saveImportLearnerResp?.statusCode == 200) {
        toast.success(
          <p className="mx-2 tx-16 d-flex align-items-center mb-0 ">{saveImportLearnerResp?.message}</p>,
          {
            position: toast.POSITION.TOP_RIGHT,
...
if (errorData?.statusCode === 400) {
  ...
  const handleSubmit = () => {
    ...
    if (selectedQuestions && selectedQuestions.length > 0) {
      ...
      dispatch(saveImportLearner(finalArray));
  ...
  return (
    <Modal show={impLer} size="xl" backdrop="static">
      <Modal.Header>
