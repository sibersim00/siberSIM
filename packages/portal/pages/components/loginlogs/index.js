...
//LOADED COMPONENT
import Adminuser from "./report/adminuserlogs";
import Instructors from "./report/instructorlogs";
import Normaluser from "./report/normaluserlogs";
...
const Master = () => {
  ...
  useEffect(() => {
    if (getSubMenus) {
      try {
        if (getSubMenus?.usertype) {
          setUserType(getSubMenus.usertype);
        }
      } catch (error) {
  ...
  return (
    <>
      ...
      <Row className="row-sm">
        ...
        <Col md={12}>
          {indexId == "tab1" && <Adminuser />}
          {indexId == "tab2" && <Instructors />}
          {indexId == "tab3" && <Normaluser />}
