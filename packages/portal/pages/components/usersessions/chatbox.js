...
useEffect(() => {
  ...
  const {
    ...
    getChatMessagesListData: state?.chatboxManage?.getChatMessagesListData?.data,
  ...
  useEffect(() => {
    if (showChat && rowData?.scenariolearnerid) {
      dispatch(getChatMessages({
        scenariolearnerid: rowData.scenariolearnerid,
      }));
    }
  ...
  useEffect(() => {
    if (getRefreshMsg && getRefreshMsg.length > 0) {
      ...
      const uniqueArr = newChat.filter((item, index, self) => index === self.findIndex(t => t.scenariolearnerchatid === item.scenariolearnerchatid));
