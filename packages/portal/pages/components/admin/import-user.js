...
import {getListOfUser,  saveImportAdUser, clearImportAdUser,clearHasError} from "../../../shared/redux/slices/admin/Users"; 
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      if (saveImportAdUserResp?.statusCode == 200) {
        toast.success(
          <p className="mx-2 tx-16 d-flex align-items-center mb-0 ">{saveImportAdUserResp?.message}</p>,
          {
            position: toast.POSITION.TOP_RIGHT,
...
if (errorData?.statusCode === 400) {
  columnDefs = [
    {
      ...
      width: 100,
      cellRenderer: (params) => {
        return params?.data?.status == 'valid' ? <div className="text-success" >{params?.data["status"]}</div> : <div className="text-secondary" >{params?.data["status"]}</div>;
      },
      filter: true,
      floatingFilter: true,
    ...
    },
    {
      ...
      width: 200,
      cellRenderer: (params) => {
        return <div>{params?.data["loginid"] || params?.data["loginid"]}</div>;
      },
      filter: true,
      floatingFilter: true,
    },
    {
      ...
      width: 200,
      cellRenderer: (params) => {
        return <div>{params?.data["First Name"] || params?.data["firstname"]}</div>;
      },
      filter: true,
      floatingFilter: true,
    },
    {
      ...
      width: 200,
      cellRenderer: (params) => {
        return <div>{params?.data["Last Name"] || params?.data["lastname"]}</div>;
      },
      filter: true,
      floatingFilter: true,
    },
    {
      ...
      width: 200,
      cellRenderer: (params) => {
        return <div>{params?.data["Email Address"] || params?.data["email"]}</div>;
      },
      filter: true,
      floatingFilter: true,
    },
    {
      ...
      width: 200,
      cellRenderer: (params) => {
        return <div>{params?.data["Phone Number"] || params?.data["mobile"]}</div>;
      },
      filter: true,
      floatingFilter: true,
    }
  ...
  columnDefs = [
    {
      ...
      width: 200,
      cellRenderer: (params) => {
        return <div>{params?.data["Login Id"] || params?.data["loginid"]}</div>;
      },
      filter: true,
      floatingFilter: true,
    },
  ...
  const formik = useFormik({
      ...
      validationSchema: yup.object().shape({
        ...
        // question_order: yup.string().when("isSelected", {
        //   is: true,
        //   then:()=> yup.string().required("Question order is required when selected"),
        //   otherwise:()=> yup.string().notRequired(),
        // }),
        // marks: yup.string().when("isSelected", {
        //   is: true,
        //   then:()=> yup.string().required("Marks are required when selected"),
        //   otherwise:()=> yup.string().notRequired(),
        // }),
      }),
                  ...
                  if (questionImport && questionImport.length == 0) {
                    ...
                    questionImport.forEach((question, index) => {
                      ...
                      if (question.correct_answer.length == 0) {
                        ...
                        question.status.push(`correct_answer cannot be an empty array.`);
                      } else if (
                      ...
                      if (definedOptionsCount < 2) {
                        validationMessages.push(
                          `Question ${index + 1}: At least two options must be provided.`
                        );
    ...
    <Modal show={impUser} size="xl" backdrop="static">
      ...
      <Form >
        <Modal.Body>
            ...
            <Form.Group as={Col} lg={12} controlId="validationFormik102" className="mb-3">
