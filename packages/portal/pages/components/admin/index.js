...
const Admin = () => {
  ...
  useEffect(()=>{
    if(getSubMenus && getSubMenus?.source=='/admin'){
    ...
    <>
      ...
      <Row className='row-sm'>
          ...
          <Card className=" custom-card mail-container task-container overflow-hidden">
            <div className="">
              <div className="p-3 mg-t-5 border-bottom">
                ...
                />
                <span className="pos-absolute tx-16 r-25" style={{top:'1.7em',zIndex:"3",opacity:"0.5"}}><i className="ion-search"></i></span>
              </div>
        ...
        <Col md={tabOrNav ? 12 : 10}>
          {selectedSubMenu && (
            <div>
              {selectedSubMenu.source === "/menus" && <Menues/>}
              {selectedSubMenu.source === "/users" && <Users/>}
              {selectedSubMenu.source === "/roles" && <Roles/>}
              {selectedSubMenu.source === "/orgnization" && <Organization />}
              {selectedSubMenu.source === "/userrolepermission" && (
                <UserRoleMapping />
              )}
            </div>
          )}
