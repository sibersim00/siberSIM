...
const Forgetpassword = () => {
  ...
  const { username, new_password, confirm_password, otp } = data;
  ...
  const { otpSuccessData, changepasswordData, orgData, getCompanyListData, errorData } =
    useSelector((state) => {
      return {
        getCompanyListData:
          state && state.authData && state.authData.getCompanyListData.data,
        otpSuccessData:
          state && state.instLoginData && state.instLoginData.forgetChangepass,
        changepasswordData:
          state && state.instLoginData && state.instLoginData.forgetSuccessData,
        orgData:
          state &&
          state.instLoginData &&
  ...
  const handleChangepassword = (e) => {
    ...
    if (data.username == "") {
      ...
      if (isLoginSuccess) {
        if (data.new_password == "") {
        ...
        else if ((data.new_password.length < 8) && (!/[A-Z]/.test(data.new_password)) && (!/[a-z]/.test(data.new_password)) && (!/[^A-Za-z0-9]/.test)) {
          toast.error(
            <p className="mx-2 tx-16 d-flex align-items-center mb-0">
          ...
          toast.error(
            <p className="mx-2 tx-16 d-flex align-items-center mb-0">
              {t("login.errormsg.password_uppercase")}
            </p>,
            {
              position: toast.POSITION.TOP_RIGHT,
              hideProgressBar: true,
          ...
          toast.error(
            <p className="mx-2 tx-16 d-flex align-items-center mb-0">
              {t("login.errormsg.password_lowercase")}
            </p>,
            {
              position: toast.POSITION.TOP_RIGHT,
              hideProgressBar: true,
            ...
            <Row className="row-sm p-0">
              ...
              >
                <div className="mt-5 pt-4 p-2 pos-absolute">
                  ...
                  <h5 className="mt-4 text-white">{t("login.forget-title")}</h5>
