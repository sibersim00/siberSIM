...
const VerifyPage = () => {
  ...
  const { errorData, hasverifyInstructorData } = useSelector((state) => {
    return {
      errorData: state?.InstructorData?.error,
      hasverifyInstructorData: state?.InstructorData?.verifyInstructorData,
    };
  });

  useEffect(() => {
    if (router.isReady && router.query.slug) {
      const instructor_useruuid = router.query.slug[0];
                ...
                {!respflag ? (
                 ...
                 >
                  ...
                  <div className="text-center text-white">
                    ...
                    <h3 className="mt-4 mb-3">
                      {hasverifyInstructorData?.message || "Account verified successfully!"}
                    </h3>
