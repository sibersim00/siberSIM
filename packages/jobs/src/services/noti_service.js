  ...
  class NotiJob {
    ...
    async getTemplateById(action) {
        let [res] = await this._db.sequelize.query(`SELECT * FROM noti_templates WHERE template_action =:_template_action`,{
          replacements: {
    ...
    async getUnprocessedNotifications() {
        let [res] = await this._db.sequelize.query(`select * from noti_logs  where is_processing = 'N'  and processon is null;`);
          return res;
      }
      
    async UpdateNotiLog(notiobject,log_id) {
        let link = notiobject.link ? notiobject.link : "";
        let [res] = await this._db.sequelize.query(`update noti_logs set body = :_body, link=:_link, is_processing = "Y", processon = now() where log_id =:_log_id AND is_processing = "N"`,{
          replacements: {
    ...
    async markAsProcessedNotifications(id) {
        let [res] = await this._db.sequelize.query(`update noti_logs set is_processing='Y', processon = now() where log_id =:_log_id`,{
          replacements: {
              _log_id: id
          }
        });
      ...
      return replaceAsync(parsedPayload, selectorRegex, async (_, selector) => {
        ...
        if (result.selector_query) {
          ...
          if(result.selector_keys == ""){
              queryResult = await this._db.sequelize.query(result.selector_query,  {type: this._db.sequelize.QueryTypes.SELECT });
            }else{
              queryResult = await this._db.sequelize.query(result.selector_query,  {  replacements: [payload[result.selector_keys]], type: this._db.sequelize.QueryTypes.SELECT });
            }
    ...
    async triggerNoti(NotiLogObject) {
      try {
          ...
          for (const field of fields) {
              if (notiConfig[field] && notiConfig[field]!='' && notiConfig[field] !=null) {
