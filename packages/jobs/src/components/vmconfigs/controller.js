  ...
  try {
    ...
    const imagesToCopy = componentImages
      .map((c) => {
        ...
        return parts ? `/uploads/components/${parts[parts.length - 1]}` : null;
      })
      .filter(Boolean);
    ...
    backupFiles.forEach((fileObj) => {
      ...
      if (fs.existsSync(srcPath)) {
        ...
        console.warn(`[Backup File Missing] ${fileName} Not Found in`, srcPath);
      }
    });
...
//         const result = await dao.getQemuConfig({
//           db,
//           ipAddress,
//           validation,
//         })(vmid);
...
//             // NEW FIELDS
//             approvalFlag: result.approvalFlag,           // true / false
//             approvalMessage: result.approvalMessage,     // auto/admin message
//           });
//         } else {
//           return res.status(500).send({
//             statusCode: 500,
